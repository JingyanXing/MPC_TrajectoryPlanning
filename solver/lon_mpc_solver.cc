...
DM LonMpcSolver(double curr_s, double curr_v, double curr_a, double ref_s, double ref_v, double ref_a,
                        double max_speed, double max_acc, double max_decel, double max_jerk){
    //预测步长2s(20步)
    const int N = 20;
    const double T = 0.1;
    ...
    Function f = Function("ocp_fun",{x, _j}, {ocp_fun}, {"state", "input"}, {"ocp_fun"});
    SX U = SX::sym("U", 1, N);
    SX X = SX::sym("X", 3, N + 1); //N+1步的系统状态，通常长度#include <algorithm>比控制多1
    //初始状态，终端状态
    SX P = SX::sym("P", 6);
    ...
    cost_f = 0;
    for(int i = 0; i < N; i++) {
        SX states_err = X(Slice(),i) - P(Slice(3,6,1));//从索引 3 开始（包含索引 3），到索引 6 结束（不包含索引 6），以步长 1 选择元素。
